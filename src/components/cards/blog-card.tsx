...
export default function BlogCard({
  ...
  const shortTitle = title?.length > 62 ? title?.slice(0, 62) + '...' : title;
  ...
  return (
    <Link href={`/blogs/${category?.name}/${slug}`}>
      <div
        key={id}
        className="group/bento shadow-input relative row-span-1 flex h-full cursor-pointer flex-col justify-between space-y-4 overflow-hidden rounded-xl border border-neutral-200 bg-white p-4 transition duration-200 hover:shadow-xl dark:border-white/[0.2] dark:bg-black dark:shadow-none"
      >
        {/* image */}
